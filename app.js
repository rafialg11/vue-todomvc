...
/**
 * The todos are the "model" of this application. In a real-world example, they would probably be
 *  loaded asynchronously, though they could also be injected via server-side rendering.
...
/**
 * All Vue applications are initialized with a `new Vue()` method call. The object that is passed 
 * to the method call is the configuration for our whole application.
 */
new Vue({
    /**
     * The "el" option indicates which HTML element the application should initially load, into.
     *  If you look at `index.html`, you will notice a div tag with the ID #app. This initial 
     * load process is also known as "bootstrapping."
     */
    el:`#app`,

    /**
     * The template for the application defines the basic structure of its HTML.
     * There are many ways to define a template in Vue, but this example uses HTML templates. If you 
     * look at the Index file, you will see an `x-template` tag with this ID. Changing the contents 
     * of that tag will change the way the application looks.
     * Other ways to define a template include using JSX or just a JavaScript string.
     */
    template:"#app-template",

    /**
     * The data method is called by the component, and the values returned become the data 
    ...
    computed:{
         ...
         * This method could be upgraded to take into accounts other things - like if the text input is 
         * valid based on the output of some validator.
         */
        submitIsDisabled(){
            return this.text == "";
        }
    ...
    methods:{
        /**
         * addTodo() takes whatever is in the text input box, and makes it into a list element. 
         * There is no need to call a `render()` method, as Vue can determine on its own what to redraw.
         */
        addTodo(){
            todos.push({
                name:this.text,
